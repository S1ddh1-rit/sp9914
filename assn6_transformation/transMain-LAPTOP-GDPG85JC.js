...
function drawShapes() {
  ...
  const baseScale = [1.5, baseHeight, 1.5];
  const columnScale = [0.6, columnHeight, 0.6];
  const topScale = [1.5, topHeight, 1.5];
  ...
  for (let i = 0; i < offsets.length; i++) {
      const x = offsets[i];
      drawCube(x, baseY, 0, ...baseScale);     // base cube
      drawCube(x, columnY, 0, ...columnScale); // column
      drawCube(x, topY, 0, ...topScale);       // top platform
  }

  // Compute platform top Y
  const platformTopY = topY + topHeight / 2;  // 0.75 + 0.1 = 0.85

  // ---- Sphere on the left ----
  {
      let sphereM = glMatrix.mat4.create();
      glMatrix.mat4.translate(sphereM, sphereM, [-3, platformTopY + 0.4, 0]); // 0.8 height → +0.4
  ...
  // ---- Teapot in the center ----
  {
      let teapotM = glMatrix.mat4.create();
      glMatrix.mat4.translate(teapotM, teapotM, [0, platformTopY + 0.42, 0]); // 0.84 height → +0.42
  ...
  // ---- Cone on the right ----
  {
      ...
      glMatrix.mat4.translate(coneM, coneM, [3, platformTopY + 0.4, 0]); // 0.8 height → +0.4
