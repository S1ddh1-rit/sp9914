...
function setUpCamera() {
  ...
  glMatrix.mat4.perspective(proj, radians(45), 1.0, 0.1, 100.0);
  gl.uniformMatrix4fv(program.uProjT, false, proj);

  let view = glMatrix.mat4.create();
  glMatrix.mat4.lookAt(view, [0, 3, 8], [0, 0, 0], [0, 1, 0]);
...
//
function drawShapes() {
  function drawCube(tx, ty, tz, sx, sy, sz) {
    let model = glMatrix.mat4.create();
    glMatrix.mat4.translate(model, model, [tx, ty, tz]);
    glMatrix.mat4.scale(model, model, [sx, sy, sz]);
  ...
  const baseY   = -2.25;
  const columnY = baseY + baseHeight + (columnHeight / 2);
  const topY    = columnY + (columnHeight / 2) + (topHeight / 2); 

  const baseScale   = [1.2, baseHeight, 1.2];
  const columnScale = [0.9, columnHeight, 0.9];
  const topScale    = [1.2, topHeight, 1.2];

  const xOffsets = [-3, 0, 3]; 
  ...
  {
    let model = glMatrix.mat4.create();
    glMatrix.mat4.translate(model, model, [-3, platformTopY + 0.4, 0]);
  ...
  {
    let model = glMatrix.mat4.create();
    glMatrix.mat4.translate(model, model, [0, platformTopY, 0]);
    glMatrix.mat4.rotateY(model, model, radians(180)); 
    glMatrix.mat4.scale(model, model, [0.6, 0.6, 0.6]);
    gl.uniformMatrix4fv(program.uModelT, false, model);
